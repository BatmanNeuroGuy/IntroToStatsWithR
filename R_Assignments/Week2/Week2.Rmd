...
```{r rdu-flights-hist}
rdu_flights <- nycflights %>% #create a new df for RDU flights
  filter(dest == "RDU")
...
sfo_feb_flights %>%
  summarise(mean_dd = mean(dep_delay), sd_dd = sd(dep_delay), n = n())
```
** It is false that no flight is delayed more than 2 hours**
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(Median = median(arr_delay), InterQuartileRange = IQR(arr_delay) )
```
'' Virgin America has the highest IQR of arr delays''
...
nycflights %>%
  ...
  arrange(desc(med_dd))
```
''December has the highest median departure delay''
...
''Median would be more reliable as the distribution of delays is skewed. BUT mean tells you that if there is a delay, which months would most likely have a significant delay, so i think both stats provide useful information.''
...
nycflights <- nycflights %>%
  mutate(arr_type = ifelse(arr_delay < 0, "ontime","delayed")) 

nycflights %>%
  group_by(dep_type) %>%
  summarise(ot_arr_rate = sum(arr_type == "ontime") / n())
